...
(
    ...
    `count` int DEFAULT NULL COMMENT '이용권 수, NULL 일 경우 무제한',
    `period` int DEFAULT NULL COMMENT '기간(일), NULL 일 경우 무제한',
...
(
    ...
    `remaining_count` int DEFAULT NULL DEFAULT '0' COMMENT '잔여 이용권 수, NULL 일 경우 무제한',
...
(
    ...
    `count` int DEFAULT NULL COMMENT '이용권 수, NULL 일 경우 무제한',
    `started_at` timestamp NOT NULL COMMENT '시작 일시',
    `ended_at` timestamp DEFAULT NULL COMMENT '종료 일시, NULL 일 경우 무제한',
    `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '생성 일시',
    `modified_at` timestamp DEFAULT NULL COMMENT '수정 일시',
...
(
    `user_group_id` varchar(20) NOT NULL COMMENT '사용자 그룹 ID',
    `user_id` varchar(20) NOT NULL COMMENT '사용자 ID',
    `user_group_name` varchar(20) NOT NULL COMMENT '사용자 그룹 이름',
    `description` varchar(20) NOT NULL COMMENT '설명',
    `created_at`timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '생성 일시',
    `modified_at` timestamp DEFAULT NULL COMMENT '수정 일시',
